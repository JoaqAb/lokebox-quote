# Texto del listado del Project Catalog

Fecha: 2026-09-17
Estado: cerrado. Unica fuente del texto del listado: de aca se copia y se pega al publicar. docs/comercial/UPWORK.md ya no lo duplica.

Reglas que cumple este texto:

- El eje es hasta tres tipos de cartel con todas sus variantes, no cantidad de familias de producto (D15).
- El precio es el mismo de la landing: USD 250 de setup mas USD 29 por mes, presentado como piso (D17). No hay tiers ni precios de add-ons.
- No se usa la palabra gratis ni ninguna promesa de prueba (D25).
- Ingles nivel B2, frases simples.

## Nombre, categoria y titulo

Categoria: Development & IT > Web Application Programming. Es donde estan todos los listados de configuradores del Catalog.

Titulo, sin repetir el "You will get" que antepone Upwork:

> an interactive 3D sign configurator with live pricing and quote capture

Motivo: "3D product configurator" es el termino con volumen y con muchos vendedores con reviews; con 0 reviews no se gana esa consulta. "sign configurator" no tiene competencia. El termino generico se cubre en el primer parrafo de la descripcion y en los tags, que tambien matchean la busqueda.

Nombre publico del producto: Lokebox Quote Builder. El nombre interno del repo sigue siendo lokebox-quote.

## Descripcion

> Your customers ask how much for a sign, and you answer one message at a time. This tool answers on your own page.
>
> The visitor picks the sign type, the size, the material, the lighting and the quantity. A 3D preview changes while they pick. The price updates on screen as a range, with your own note next to it. When they are ready, they send you the request by WhatsApp or by form, with every option already filled in, and they can print the quote sheet.
>
> The page carries your logo, your colors and your texts. Your prices are loaded and checked with you before it goes online.
>
> What you get covers up to three sign types, with all their options: face lit and back lit panels, totems, and channel letters with the text your customer types. It is not a template with one product in it.

## Que incluye

> - Quote tool with your logo, your colors and your texts.
> - Up to three sign types, with all their options.
> - 3D preview that changes while the visitor picks options.
> - Price on screen as a range, with your own note.
> - Leads with the full configuration, by WhatsApp or by form.
> - Printable quote sheet.
> - Your prices loaded and checked with you.

## Abono, los siete puntos

> Included every month:
>
> - Prices kept up to date, up to two updates per month.
> - Your page online, with your address and its certificate.
> - Orders saved with the full configuration.
> - Small changes at no cost.
> - Product improvements included.
> - Support with an answer within one working day.
> - Cancel whenever you want and keep your data.

## Precio

> From USD 250 setup, plus USD 29 per month.
>
> Anything beyond that scope is quoted case by case.

Si el Catalog obliga a cargar un precio unico de entrega, se carga el setup de USD 250 y el abono se explica en la descripcion y se contrata aparte.

## Lo que se construye por mas

> Built on request, quoted case by case:
>
> - More sign types, or another product family.
> - More complex price rules.
> - Your photos of real jobs in the preview.
> - Another language on the same page.

## Que necesito del cliente

> - Your logo and your colors.
> - Your WhatsApp number or your email.
> - Your prices, in the spreadsheet I send you.

## Enlace a la demo

> See how it works: https://quote.lokebox.com
>
> This is a demo to show you the product. It is not set up for your daily work.

## Material visual

Video de 30 segundos y capturas de desktop y mobile de la demo EN, del paso de Canal C del jueves 17. Orden del video: cartel base, cambio de material, cambio de iluminacion, precio moviendose, lead.

## Tags

A revisar contra lo que Upwork permita seleccionar al publicar:

3D Product Configurator · Three.js · Quote Calculator · React · Signage
