...
- TAREA_019 · escenario del modo cartel: el criterio 2 de DONE quedó rechazado por contraste. Entra `--q-stage` como fondo del marco solo en modo cartel, la sombra de apoyo pasa a un color de escena casi negro, y la sublínea de los dos logos se agranda. El relleno del panel salió del alcance. Detalle en docs/tareas/TAREA_019_escenario_modo_cartel.md.
  - Aceptación: build sin warnings con la app a no más de 2 kB sobre 420,06 kB y el vendor 3D en 963,55 kB; lint limpio y tests en verde con los casos nuevos de tema y de sombra; en las seis capturas de modo cartel la luminancia debajo del panel y de las letras igual o menor al fondo del marco; las siete de modo vista con la foto cubriendo el marco, sin borde duro de sombra y con las tres comparaciones de luminancia de SPEC 12; la sublínea del logo legible en desktop y mobile; SPEC 1.16; push y deploy verificado.
- TAREA_013 · landing mínima en `/` con JSON propio validado, rutas de cliente con `React.lazy` y chunk `react-vendor` separado del vendor 3D. Detalle en docs/tareas/TAREA_013_landing.md.
  - Aceptación: G1 a G6 y C1 a C13. C1: build sin warnings, three-vendor por debajo de 1000 kB, react-vendor por debajo de 250 kB, suma de chunks de app por debajo de 500 kB. C2: tsc y lint limpios. C3: 205 tests previos sin editar más casos de landingConfig y landingTheme. C4: sin guiones largos. C5: `/` con loadEventEnd por debajo de 2000 ms en tres corridas sin cache. C6: `/` no pide three-vendor ni assets del preview. C7: `/d/northline` con peor caso de después no más de 300 ms sobre el de antes (86 ms). C8: panel y preview aparecen juntos. C9: los dos botones llevan a las demos y renderizan completas. C10: cero strings de UI en `src/landing`. C11: 390 px sin scroll horizontal y capturas `landing-390`, `landing-768` y `landing-1440` en el set de 21. C12: cero POST a supabase.co en `/`. C13: React no duplicado dentro de three-vendor.
