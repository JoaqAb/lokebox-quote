...
// --q-stage, con el logo del cliente y su loadingLabel. El progreso es el real del
...
export function LoadingScreen({ brand, done }: LoadingScreenProps) {
  ...
  return (
    <div
      ...
      aria-busy={!done}
      aria-hidden={done}
      className={`absolute inset-0 flex flex-col items-center justify-center gap-5 bg-[var(--q-stage)] transition-opacity duration-300 motion-reduce:transition-none ${done ? 'pointer-events-none opacity-0' : 'opacity-100'}`}
    >
      ...
      <div className="flex w-2/5 max-w-xs min-w-40 flex-col items-center gap-3">
        <div
          ...
          aria-valuemax={100}
          aria-valuenow={pct}
          className="h-1 w-full overflow-hidden rounded-full bg-[color-mix(in_srgb,var(--q-bg)_22%,transparent)]"
        >
          <div className="h-full rounded-full bg-[var(--q-accent)]" style={{ width: `${String(pct)}%` }} />
        </div>
        <span className="text-xs font-semibold tracking-[0.18em] text-[var(--q-bg)] uppercase">{brand.label}</span>
      </div>
    </div>
  )
}
