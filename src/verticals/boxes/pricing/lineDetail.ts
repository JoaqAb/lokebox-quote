...
import { areaUnitSymbol, formatArea, formatCurrency, formatInteger } from '../../../core/pricing/format'
...
export function boxBreakdownCaption(config: BoxesConfig, result: BoxPriceResult): string {
  return buildWhatsappMessage(config.texts.perBoxCaption, { quantity: formatInteger(result.quantity, config.locale) })
}
